...
class DCRNNSupervisor:
    def __init__(self, adj_mx, **kwargs):
        ...
        if self._epoch_num > 0:
            self.load_model(self._train_kwargs.get('model_path', 'models'))
        self.model_path = self._train_kwargs.get('model_path', 'models')
    ...
    def _get_log_dir(kwargs):
        ...
        if log_dir is None:
            ...
            run_id = 'dcrnn_%s_%d_h_%d_%s_lr_%g_bs_%d/' % (
    ...
    def save_model(self, epoch, model_path='models'):
        ...
        return model_path + '/epo%d.tar' % epoch

    def load_model(self, model_path='models'):
        print(model_path + '/epo%d.tar' % self._epoch_num)
        self._setup_graph()
    ...
    def compute_embedding(self, dataset='val', batches_seen=0):
        """
        Computes mean L1Loss
        :return: mean L1Loss
        """
        with torch.no_grad():
            ...
            for _, (x, y) in enumerate(val_iterator):
                ...
                encoder_hidden_state = self.dcrnn_model.encoder(x)
                encoder_hidden_state = encoder_hidden_state[-1] # get embedding from the last layer
                encoder_hidden_state = encoder_hidden_state.detach().cpu().numpy()
                for embedd in encoder_hidden_state[:]:
    ...
    def _train(self, base_lr,
        ...
        for epoch_num in range(self._epoch_num, epochs):
            ...
            if val_loss < min_val_loss:
                wait = 0
                if save_model:
                    model_file_name = self.save_model(epoch_num, self.model_path)
                    self._logger.info(
